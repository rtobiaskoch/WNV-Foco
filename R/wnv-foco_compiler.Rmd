...
mdata_co = mdata_co0 %>%
  filter(!is.na(date)) #696 all the same as whats in the mdata_old
                       #72 have missing dates meaning they are from the trap page 23-09-15
...
mdata_c = mdata %>%
  ...
  mutate(CO = case_when(state == "CO" & str_detect(mdata$accession, "^CSU") ~ "FoCo",
                        state == "CO"                              ~ county,
                        TRUE                                       ~ "Not CO"
                        ),
