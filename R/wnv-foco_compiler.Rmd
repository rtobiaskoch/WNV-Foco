...
state_collapse = "AL|CAN|CT|MN|NY|OH|MD|GA|LA|VA|WI|MI|WA"
...
```{r}
path = 'data_input/2013-2021 WNV Surveillance Trap Consolidation (from Kaitlynn).xlsx'

trap = read_excel(path, 
...
#### * OLD METADATA (ORIGINAL BUILD)

```{r}
mdata_co_old0 = read_excel("data_input/Fort Collins WNV Metadata.xlsx")
#NOTES
# problem with this data is it doesn't include the trap ID so I will need to use the new data

mdata_co_old = mdata_co_old0 %>%
  clean_names() %>%
...
mdata_co_new0 <- map_df(sheet_names[2:10], ~ read_excel(path, #introduced in Trap data read in
                                                 sheet = .x, 
                                                 col_types = "text",
                                                 skip =1))

mdata_co_new = mdata_co_new0 %>%
  clean_names() %>%
...
mdata_co_old_t = mdata_co_old %>%
  select(accession, date, host)
  
  
mdata_co0 = full_join(mdata_co_new, mdata_co_old_t, by = c("accession")) %>%
  mutate(date = as.Date(date))

mdata_co = mdata_co0 %>%
  filter(!is.na(date)) #696
  

rm(mdata_co_old_t)

rm(mdata_co_new, mdata_co_new0,
   mdata_co_old, mdata_co_old0)
```
...
co_not_other = anti_join(mdata_co, mdata_other, by = "accession")

mdata = full_join(mdata_other, mdata_co_t, by = "accession") #3553
rm(mdata_co_t, mdata_other0)
```

#### * CLEAN AND FORMAT FOR NEXTSTRAIN
```{r}
mdata_c = mdata %>%
  distinct(accession, .keep_all = T) %>% #3541 no duplicates were found
  dplyr::rename(county = city) %>% #essentially county
  mutate(CO = if_else(state == "CO", "CO", "not CO"),
        division = paste0(state, "/", county), #to match format of 20 yr west nile that has lat and long already
         ...
         strain = paste(accession, year, country, division, lineage, sublineage, host, sep = "/")
          ) %>%
  select(-county) %>%
...
#### * FIND MISSING CO SEQUENCES
remove sequences that are not in the metadata
```{r}
#remove any sequences in fasta file that are not in the metadata
fasta_co_c = filter_fasta(fasta_co, mdata_co0$accession) #695 all samples in fasta are in mdata_co_new
...
#### * COMBINE & DEDUP
combines CO fasta and the rest of the build fasta
```{r}
...
fasta_c = filter_fasta(fasta, mdata_c$accession)


```


#### * REMOVE LT<80% COVERAGE
...
fasta_c_cmplt = filter_fasta(fasta_c, fasta_c_char80$accession)
...
write.csv(mdata_c_cmplt, "data_output/mdata_c_cmplt.csv")
write.csv(mdata_c_cmplt, "../data_test/metadata.csv")
...
fasta_ns = fasta_name_switch(fasta_c_cmplt, names(fasta_c_cmplt), mdata_c_cmplt$strain)
...
col_country = data.frame("geo" = rep("country", n), 
                    "Country" = unique(c_cmplt_mdata_ds$Country),
                    "col" = col_vector) %>%
  rename(x = Country) #rename so rbind works

n2 = length(unique(c_cmplt_mdata_ds$region))

col_region = c_cmplt_mdata_ds %>%
  select(Country, region) %>%
  rename(x = region) %>% #rename so rbind works
  distinct_all() %>%
  left_join(col_country, by = c("Country" = "x")) %>% # get colors
  select(-Country) %>% #drop country to conform to nextstrain format
  mutate(geo = "region")

col_df = rbind.data.frame(col_country,  col_region)

write.csv(col_df, "data_mid/color_generator.csv", row.names = F)
write.csv(col_df, "../WNV-nextstrain/config/color_generator.csv", row.names = F)

rm(n, qual_col_pals, col_vector, col_country)
```
